...
export const setAddModalKeyValue = createAction("restaurants/setAddModalKeyValue");
export const sortRestaurant = createAction("restaurants/sortRestaurant");
export const filterRestaurantByName = createAction("restaurants/filterRestaurantByName");
...
export const init = () => {
  return dispatch => {
      ...
      navigator.geolocation.getCurrentPosition((position) => dispatch(setGeoLocation(position)) )
      }
...
const ACTION_HANDLERS = {
  ...
  [setGeoLocation]: (state, action) => {
        ...
        if(position){
        state.addModalContent.geoCode =[ position.coords.latitude, position.coords.longitude]
  ...
  [addRestaurant]: (state, action) => {
    if(state.restaurants.filter( item => item.name == action.payload.name).length == 0) {
  ...
  [selectRestaurant]: (state, action) => {
    let selectedRestaurant = state.restaurants.filter(res => res.id == action.payload)[0];
  ...
  [sortRestaurant]: (state, action) => {
    if(state.sortDesc)
    state.restaurants.sort((res1, res2) => res2.name.localeCompare(res1.name));
    else state.restaurants.sort((res1, res2) => res1.name.localeCompare(res2.name));
